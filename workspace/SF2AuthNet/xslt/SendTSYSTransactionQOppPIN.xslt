...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="iwtransformationserver">
		...
		<xsl:variable name="CurrentStartDateTime">
			<xsl:if test="iwrecordset/transaction[@name='sessionvars']/datamap[@name='sessionvars']/data/row/col[@name='CurrentStartDateTime']">
				<xsl:value-of select="iwrecordset/transaction[@name='sessionvars']/datamap[@name='sessionvars']/data/row/col[@name='CurrentStartDateTime']"/>
			</xsl:if>
		</xsl:variable>
		<xsl:variable name="PostDate">
			<xsl:value-of select="concat(substring($CurrentStartDateTime, 6, 2), substring($CurrentStartDateTime, 9, 2), substring($CurrentStartDateTime, 1, 4))"/>
		</xsl:variable>
			...
			<xsl:for-each select="iwrecordset/transaction[@name='sessionvars']/datamap[@name='sessionvars']/data/row/col[starts-with(@name,'QDSN')]">
				...
				<xsl:if test="$QDSNGVal!='' and ($QDSNG='QDSN0' or $CIMMode!='0')">
					...
					<xsl:for-each select="//iwtransformationserver/iwrecordset/transaction/datamap[@name='retrieveacct']/data/rowset[@name='Envelope']/rowset[@name='Body']/rowset[@name='queryResponse']/rowset[@name='result']/rowset[@name='records']">
						<xsl:if test="row/col[@name='Id'] and starts-with($CurrentFlowId, 'SFTransactions2TSYS') and ($SchedCCType!='Cust' or rowset[@name='Scheduled_Payments__r']/rowset[@name='records']/row/col[@name='Payment_Amount__c'])">
...
Version 2.10 Copyright 2016 TransFirst, LLC. All rights reserved. 155  
  "achEcheck": {     "bankRtNr": "121212120",     "acctNr": "987654321",     "acctType": 0,     "seccCode": 0,   }, } </xsl:text>
								...
								<xsl:if test="$APIType='POST'">
									<xsl:if test="not(contains($APIType00, ':'))">%pause=900000%</xsl:if>GatewayID=<xsl:value-of select="$APILogin"/>&amp;RegKey=<xsl:value-of select="$TransKey"/>&amp;Amount=<xsl:value-of select="$Charge"/>&amp;CustRefID=<xsl:value-of select="$SFId"/>&amp;PostDate=<xsl:value-of select="$PostDate"/>`</xsl:if>
								<!--</xsl:choose>
