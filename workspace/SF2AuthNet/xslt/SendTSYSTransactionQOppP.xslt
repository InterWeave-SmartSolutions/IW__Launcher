...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="iwtransformationserver">
		...
		<xsl:if test="not(contains($APIType0, ':'))">
									...
									<xsl:variable name="Charge0">
										<xsl:choose>
											<xsl:when test="$ACH='true'">
												<xsl:value-of select="string(number($TotBal)*100)"/>
											</xsl:when>
...
Version 2.10 Copyright 2016 TransFirst, LLC. All rights reserved. 155  
  "achEcheck": {     "bankRtNr": "121212120",     "acctNr": "987654321",     "acctType": 0,     "seccCode": 0,   }, } </xsl:text>
									...
									<xsl:variable name="FullName0" select="concat($FirstName, ' ', $LastName)"/>
									<xsl:variable name="FullName">
										<xsl:if test="$FullName0!=' '">
											<xsl:value-of select="$FullName0"/>
										</xsl:if>
									</xsl:variable>
									<xsl:if test="$APIType='POST'">GatewayID=<xsl:value-of select="$APILogin"/>&amp;RegKey=<xsl:value-of select="$TransKey"/>&amp;<xsl:choose>
											<xsl:when test="$ACH='true'">SecCode=1&amp;AccountNumber=<xsl:value-of select="$AcctNum"/>&amp;RoutingNumber=<xsl:value-of select="$ABA"/>&amp;Descriptor=&amp;Amount=<xsl:value-of select="$Charge"/>
											</xsl:when>
											<xsl:otherwise>IndustryCode=2&amp;AccountNumber=<xsl:value-of select="$CCNumber"/>&amp;CVV2=<xsl:value-of select="$CVM"/>&amp;ExpirationDate=<xsl:value-of select="$CCExpDate"/>&amp;Track1Data=&amp;Track2Data=&amp;Amount=<xsl:value-of select="$Charge"/>&amp;TaxIndicator=&amp;TaxAmount=&amp;PONumber=&amp;ShipToZip=<xsl:value-of select="$PostCode"/>
											</xsl:otherwise>
										</xsl:choose>&amp;CustRefID=<xsl:value-of select="$SFId"/>&amp;FullName=<xsl:value-of select="$FullName"/>&amp;Address1=<xsl:value-of select="$Address"/>&amp;Address2=&amp;City=<xsl:value-of select="$City"/>&amp;State=<xsl:value-of select="$State"/>&amp;Zip=<xsl:value-of select="$PostCode"/>&amp;PhoneNumber=<xsl:value-of select="$Phone"/>&amp;Email=<xsl:value-of select="$Email"/>`</xsl:if>
									<!--</xsl:choose>
