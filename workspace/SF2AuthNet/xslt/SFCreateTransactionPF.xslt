...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="iwtransformationserver">
		...
		<xsl:for-each select="iwrecordset/transaction/datamap[@name='payauth']/data/rowset[@name='XMLPayResponse']/rowset[@name='ResponseData']/rowset[@name='TransactionResults']/rowset[@name='TransactionResult']/row">
			<xsl:variable name="R1" select="col[@name='Result']"/>
			<xsl:variable name="R4" select="col[@name='Message']"/>
			<xsl:variable name="R7" select="col[@name='PNRef']"/>
			...
			<xsl:variable name="AppStatus">
				<xsl:choose>
					...
					<xsl:when test="$R1=12">Declined</xsl:when>
					<xsl:when test="$R1=13">Held for Review</xsl:when>
			...
			<xsl:variable name="RoutingBank">PayFlow</xsl:variable>
			<!--<xsl:choose>
					<xsl:when test="//iwtransformationserver/iwrecordset/transaction[@name='RetrieveSFAcctDataLId']">Monterey</xsl:when>
					<xsl:when test="//iwtransformationserver/iwrecordset/transaction[@name='RetrieveSFSODataLId']">Authorize.net</xsl:when><xsl:when test="//iwtransformationserver/iwrecordset/transaction[@name='RetrieveSFSODataLId']">Authorize.net</xsl:when>
				</xsl:choose>
			</xsl:variable>-->
				...
				<soap:Body>
					<create xmlns="urn:partner.soap.sforce.com">
						<sObjects>
							...
							<xsl:choose>
								<xsl:when test="//iwtransformationserver/iwrecordset/transaction[@name='PayPFQOpp']">
									<Sales_Order__c>
