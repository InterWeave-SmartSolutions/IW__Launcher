...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="iwtransformationserver">
		...
		<xsl:variable name="OrlandoWhere">
			<xsl:choose>
				<xsl:when test="number($CurrentDay)=1 or number($CurrentDay)=15">Source_Location__c='ORLANDO' </xsl:when>
				<xsl:otherwise>(Source_Location__c='ORLANDO')  and (SF_Deal__c=true or RGM_Deal__c=true)</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
			...
			<soap:Body>
				<query xmlns="urn:partner.soap.sforce.com">
					<queryString>Select Id, Name, Gross_Monthly_Charge__c, Credit_Card_Client__c, CC_Exp_Date_00_00__c, FirstName, LastName, Number_of_Declines__c, ACH__c, BillingPostalCode, BillingStreet, BillingCity, BillingState, BillingCountry, Phone, PersonEmail, Client_Acct__c, Routing__c, Account_Type__c, Total_Balance__c from Account where Last_Authorized_Payment_Date__c!=TODAY and PersonHasOptedOutOfEmail=false and ACH__c=true and Sent_To_Mont__c=false and (Client_Status__c='Active' or Client_Status__c='Cancel No Pay') and Special_Billing_2__c=false and (Card_Bills_On_2__c like '<xsl:value-of select="$CurrentDay"/>%'<xsl:value-of select="$Q0"/>) and ((<xsl:value-of select="$OrlandoWhere"/>) or ((Source_Location__c='PGA') and  (Gross_Monthly_Charge__c &gt; 500))) and Paid_By_Check__pc=false 
						</queryString>
				</query>
			</soap:Body>
		</soap:Envelope>
