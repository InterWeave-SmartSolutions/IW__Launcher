	...
	<xsl:template match="iwtransformationserver">
		...
		<xsl:for-each select="iwrecordset/transaction/datamap[@name='payauth']/data/row">
			...
			<xsl:variable name="AppStatus">
				<xsl:choose>
					<xsl:when test="$R1='APPROVED'">Approved</xsl:when>
					<xsl:when test="$R1='DECLINED'">Declined</xsl:when>
					<xsl:when test="$R1='BLOCKED'">Held for Review</xsl:when>
			...
			<xsl:variable name="RoutingBank">First Data</xsl:variable>
			<!--<xsl:choose>
