	...
	<xsl:template match="iwtransformationserver">
		...
		<xsl:variable name="TransKey" select="iwrecordset/transaction[@name='sessionvars']/datamap[@name='sessionvars']/data/row/col[@name='QBPswd0']"/>-->
			...
			<xsl:variable name="Mapping01">
				<xsl:choose>
					<xsl:when test="iwrecordset/transaction[@name='sessionvars']/datamap[@name='sessionvars']/data/row/col[@name='__Param__Mapping01']">
					...
					<xsl:when test="starts-with($APIType, 'CARDPOINT|')">
						<xsl:value-of select="translate(substring-after($APIType, 'CARDPOINT|'), '/', ':')"/>
					</xsl:when>
				</xsl:choose>
			</xsl:variable>
			<xsl:variable name="Mapping01SF">
				<xsl:if test="$Mapping01!='' and contains($Mapping01, ':')">
					<xsl:value-of select="substring-before($Mapping01, ':')"/>
				</xsl:if>
			</xsl:variable>
			<xsl:variable name="Mapping01CC">
				<xsl:if test="$Mapping01!='' and contains($Mapping01, ':')">
					<xsl:value-of select="substring-after($Mapping01, ':')"/>
				</xsl:if>
			</xsl:variable>
			<xsl:for-each select="iwrecordset/transaction[@name='sessionvars']/datamap[@name='sessionvars']/data/row/col[starts-with(@name,'QDSN')]">
				...
				<xsl:if test="$QDSNGVal!='' and ($QDSNG='QDSN0' or $CIMMode!='0')">
					...
					<xsl:for-each select="//iwtransformationserver/iwrecordset/transaction/datamap[@name='retrieveacct']/data/rowset[@name='Envelope']/rowset[@name='Body']/rowset[@name='queryResponse']/rowset[@name='result']/rowset[@name='records']">
						<xsl:if test="row/col[@name='Id'] and ((not(starts-with($CurrentFlowId, 'SFTransactions2TSYS')) and not(starts-with($CurrentFlowId, 'SFTransactions2CardP'))) or $SchedCCType!='Cust' or rowset[@name='Scheduled_Payments__r']/rowset[@name='records']/row/col[@name='Payment_Amount__c'])">
							...
							<xsl:if test="$MultURL!='__%UNKNOWN_URL%__'">
								...
								<xsl:variable name="Charge0">
									<xsl:choose>
										<xsl:when test="(starts-with($CurrentFlowId, 'SFTransactions2TSYS') or starts-with($CurrentFlowId, 'SFTransactions2CardP'))  and $SchedCCType='Cust'">
								...
								<xsl:variable name="MerchId">
									<xsl:choose>
										<xsl:when test="contains($APILogin, '|')">
											<xsl:value-of select="substring-before($APILogin, '|')"/>
										</xsl:when>
										<xsl:otherwise>
								...
								<xsl:variable name="Mapping01Val">
									<xsl:if test="$Mapping01!=''">
										<xsl:value-of select="row/col[@name=$Mapping01SF]"/>
									</xsl:if>
								</xsl:variable>
								<xsl:if test="starts-with($APIType, 'CARDPOINT')">{
  "merchid": "<xsl:value-of select="$MerchId"/>",
  "orderid": "<xsl:value-of select="$SFId"/>",
  "account": "<xsl:value-of select="$CCNumber"/>",
  "expiry": "<xsl:value-of select="$CCExpDate"/>",
  "cvv2": "<xsl:value-of select="$CVM"/>",
  "postal": "<xsl:value-of select="$PostCode"/>",
  "ecomind": "E",
  "amount": "<xsl:value-of select="$Charge"/>",
  "name": "<xsl:value-of select="concat($FirstName, ' ', $LastName)"/>",
  "address": "<xsl:value-of select="$Address"/>",
  "city": "<xsl:value-of select="$City"/>",
  "region": "<xsl:value-of select="$State"/>",
  "country": "US",<!--"profile": "Y",-->
  "capture": "Y",
  "receipt": "Y"<xsl:if test="$Mapping01!=''">,
"userfields":
    {
        "<xsl:value-of select="$Mapping01CC"/>": "<xsl:value-of select="$Mapping01Val"/>"
    }</xsl:if>}`<!--<xsl:text>{   "merc": {     "id": "</xsl:text>
									<xsl:value-of select="$APILogin"/>
