...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="iwtransformationserver">
		...
		<xsl:variable name="OFURLM">
			...
			<xsl:choose>
				<xsl:when test="starts-with($APIType, 'CARDPOINT') and $CreditMode='REFUND'">
					<xsl:text>refund</xsl:text>
				</xsl:when>
				<xsl:when test="$APIType='CARDPOINT' and $CreditMode='VOID'">
				...
				<xsl:when test="$APIType='WS'">
					<xsl:text>/SendTran</xsl:text>
				</xsl:when>
				<xsl:when test="$APIType='POST' and //iwtransformationserver/iwrecordset/transaction/datamap[@name='retrieveacct']/data/rowset[@name='Envelope']/rowset[@name='Body']/rowset[@name='queryResponse']/rowset[@name='result']/rowset[@name='records']/row/col[@name='ACH__c'] and //iwtransformationserver/iwrecordset/transaction/datamap[@name='retrieveacct']/data/rowset[@name='Envelope']/rowset[@name='Body']/rowset[@name='queryResponse']/rowset[@name='result']/rowset[@name='records']/row/col[@name='ACH__c']='true'">
					<xsl:text>/ACHSale</xsl:text>
				</xsl:when>
				<xsl:when test="$APIType='POST'">
					<xsl:text>/CreditCardSale</xsl:text>
				</xsl:when>
			</xsl:choose>
			<xsl:if test="contains($QDSN0, ' %STP:')">
