		...
		<xsl:variable name="OFURLM">
			...
			<xsl:choose>
				<xsl:when test="$APIType='WS'">
					<xsl:text>/RefundOrReturn</xsl:text>
				</xsl:when>
				<xsl:when test="$APIType='POST' and $CreditMode='REFUND'">
					<xsl:text>/CreditCardRefund</xsl:text>
				</xsl:when>
				<xsl:when test="$APIType='POST' and $CreditMode='VOID'">
					<xsl:text>/CreditCardVoid</xsl:text>
				</xsl:when>
			</xsl:choose>
