...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="iwtransformationserver">
		...
		<xsl:variable name="APIType">
			<xsl:choose>
				<xsl:when test="$APIType0=''">WS</xsl:when>
				<xsl:otherwise>
				...
				<xsl:for-each select="//iwtransformationserver/iwrecordset/transaction/datamap[@name='retrieveacct']/data/rowset[@name='Envelope']/rowset[@name='Body']/rowset[@name='queryResponse']/rowset[@name='result']/rowset[@name='records']">
					<xsl:if test="row/col[@name='Id']">
						...
						<xsl:if test="$MultURL!='__%UNKNOWN_URL%__'">
							...
							<xsl:variable name="Email" select="rowset[@name='Client__r']/row/col[@name='Billing_Email__c']"/>
							<xsl:variable name="Charge0" select="string(number(row/col[@name='Amount__c'])*100)"/>
							...
							<xsl:variable name="PostCode" select="rowset[@name='Client__r']/row/col[@name='Billing_Postal_Code__c']"/>
							...
							<xsl:choose>
								<xsl:when test="$APIType='POST'">GatewayID=<xsl:value-of select="$APILogin"/>&amp;RegKey=<xsl:value-of select="$TransKey"/>&amp;Amount=<xsl:value-of select="$Charge"/>&amp;tranNr=<xsl:value-of select="$TransId"/>&amp;Email=<xsl:value-of select="$Email"/>`</xsl:when>
								<xsl:when test="$APIType='CARDPOINT'">{
  "retref": "<xsl:value-of select="row/col[@name='Retrieval_Reference_Number__c']"/>",
  "merchid": "<xsl:value-of select="$MerchId"/>",
  "amount": "<xsl:value-of select="$Charge"/>"  
}`</xsl:when>
							</xsl:choose>
							<!--x_delim_data=TRUE&amp;x_delim_char=,&amp;x_version=3.1&amp;x_login=<xsl:value-of select="$APILogin"/>&amp;x_tran_key=<xsl:value-of select="$TransKey"/>&amp;x_type=<xsl:value-of select="$CreditMode"/>&amp;x_trans_id=<xsl:value-of select="$TransId"/>&amp;x_first_name=<xsl:value-of select="$FirstName"/>&amp;x_last_name=<xsl:value-of select="$LastName"/>&amp;x_address=<xsl:value-of select="$Address"/>&amp;x_zip=<xsl:value-of select="$PostCode"/>&amp;x_amount=<xsl:value-of select="$Charge"/>&amp;x_card_num=<xsl:value-of select="$CCNumber"/>&amp;x_cust_id=<xsl:value-of select="$ParSFId"/>&amp;x_test_request=<xsl:value-of select="$TestReq"/>&amp;x_relay_response=FALSE&amp;-->
						</xsl:if>
					</xsl:if>
				</xsl:for-each>
			</xsl:if>
