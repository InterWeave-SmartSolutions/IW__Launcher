...
Then restart Tomcat via `_internal/stop_webportal.bat` and `_internal/start_webportal.bat`.
...
| `_internal/` | Extend existing script patterns. Do not rename or restructure. |
...
1. Start Tomcat: `_internal/start_webportal.bat`
...
3. **Windows-centric**: Primary scripts are `.bat` files. Linux/Mac equivalents in `_internal/` are less tested.
