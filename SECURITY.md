...
**Mitigation**: This file is excluded from git via `.gitignore` (as `context.xml`). Ensure it is never committed. Use `_internal/SETUP_DB_*.bat|sh` scripts to generate it from `.env`.
