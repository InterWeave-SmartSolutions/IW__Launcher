-- =============================================================================
-- IW_IDE Monitoring Schema Migration
-- Migration: 005_monitoring_schema.sql
-- Date: 2026-01-09
-- Description: Adds comprehensive monitoring tables for real-time integration
--              monitoring dashboard. Extends existing schema with transaction
--              execution tracking, metrics aggregation, and alerting capabilities.
-- =============================================================================
...
-- Set default storage engine to InnoDB for transaction support and foreign keys
SET default_storage_engine = InnoDB;

-- -----------------------------------------------------------------------------
-- SECTION 1: TRANSACTION EXECUTION TRACKING
...
CREATE TABLE IF NOT EXISTS transaction_executions (
    ...
    -- Foreign key constraints (with proper error handling for missing references)
    CONSTRAINT fk_exec_company FOREIGN KEY (company_id) REFERENCES companies(id) ON DELETE CASCADE,
    CONSTRAINT fk_exec_project FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE SET NULL,
    CONSTRAINT fk_exec_transformation FOREIGN KEY (transformation_id) REFERENCES transformations(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS transaction_payloads (
    ...
    CONSTRAINT fk_payload_execution FOREIGN KEY (execution_id) REFERENCES transaction_executions(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS transaction_metrics (
    ...
    -- Foreign keys
    CONSTRAINT fk_metrics_company FOREIGN KEY (company_id) REFERENCES companies(id) ON DELETE CASCADE,
    CONSTRAINT fk_metrics_project FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS alert_rules (
    ...
    CONSTRAINT fk_alert_rules_project FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS webhook_endpoints (
    ...
    CONSTRAINT fk_webhook_user FOREIGN KEY (created_by_user_id) REFERENCES users(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS alert_history (
    ...
    -- Foreign keys
    CONSTRAINT fk_alert_history_rule FOREIGN KEY (alert_rule_id) REFERENCES alert_rules(id) ON DELETE CASCADE,
    CONSTRAINT fk_alert_history_execution FOREIGN KEY (execution_id) REFERENCES transaction_executions(id) ON DELETE SET NULL,
...
-- Insert default monitoring settings (idempotent)
...
-- -----------------------------------------------------------------------------
-- SECTION 5: VIEWS FOR CONVENIENT QUERIES
...
-- =============================================================================
-- MIGRATION COMPLETE
-- Schema version updated to include monitoring capabilities
-- =============================================================================

-- Update schema version in settings
INSERT INTO settings (setting_key, setting_value, description) VALUES
    ('db_schema_version', '2.1.0', 'Database schema version with monitoring support')
ON DUPLICATE KEY UPDATE
    setting_value = '2.1.0',
    updated_at = CURRENT_TIMESTAMP;

-- Display completion message
SELECT 'Monitoring schema migration completed successfully' AS status,
       'Tables: transaction_executions, transaction_payloads, transaction_metrics, alert_rules, webhook_endpoints, alert_history' AS tables_created,
       'Views: v_recent_executions, v_system_health, v_active_alerts' AS views_created;
